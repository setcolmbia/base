...
class Preview extends Database{
    ...
    public function all(){
        try{
            $result = parent::connect()->prepare("SELECT * FROM ".PREFIX."_preview");
            $result->execute();
    ...
    public function save($data){
        ...
        try{
            $result = parent::connect()->prepare("INSERT INTO ".PREFIX."_preview (name, accountcode, call_limit, cid_number, context, fullname, mailbox, secret, username, allow, active, userCreate, dateCreate) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,NOW())");
    ...
    public function find($id){
        try{
            $result = parent::connect()->prepare("SELECT * FROM ".PREFIX."_preview WHERE id = ?");
    ...
    public function updateR($data){
        ...
        try{
            $result = parent::connect()->prepare("UPDATE ".PREFIX."_preview SET name=?, accountcode=?, call_limit=?, cid_number=?, context=?, fullname=?, mailbox=?, secret=?, username=?, allow=?, active=?, userUpdate = ?, dateUpdate = NOW() WHERE id = ?");
            $result->bindParam(1, $data['name'], PDO::PARAM_STR);
            $result->bindParam(2, $data['context'], PDO::PARAM_STR);
            $result->bindParam(3, $data['call_limit'], PDO::PARAM_STR);
            $result->bindParam(4, $data['name'], PDO::PARAM_STR);
    ...
    public function deleteR($data){
        try{
            $result = parent::connect()->prepare("DELETE FROM ".PREFIX."_preview WHERE id = ?");
